...
library(readr)
siteSpecificCov <- read.csv("C:/Users/OYDHAL001/Desktop/RangeDynamics/processedCovariates/siteSpecificCov2008_2009.csv") %>%
...
ggplot() +
  ...
  theme_minimal() +
  labs(
    title = "Predicted occupancy map (2008/09)"
  ) +
...
seasonalSiteCov <- read_csv("processedCovariates/seasonalSiteCov2010_11.csv") %>%
  rename(Pentad = pentad)%>%
  select(-c(1))
...
validationData$urban_prop <- validationData$perc_urban/100
validationData[is.na(validationData)] <- 0
validationData$predictions <- predict(model10, newdata = validationData, type = "response")
summary(validationData$predictions)
...
ggplot() +
  ...
  theme_minimal() +
  labs(
    title = "Predicted occupancy map (2010/11)"
  ) +
...
# load and merge site-specific and seasonal covariate data:
library(readr)
siteSpecificCov <- read.csv("C:/Users/OYDHAL001/Desktop/RangeDynamics/processedCovariates/siteSpecificCov2012_2015.csv") %>%
  rename(Pentad = pentad)%>%
  select(-c(1))

seasonalSiteCov <- read_csv("processedCovariates/seasonalSiteCov2012_13.csv") %>%
  rename(Pentad = pentad)%>%
  select(-c(1))
...
validationData$urban_prop <- validationData$perc_urban/100
validationData[is.na(validationData)] <- 0
validationData$predictions <- predict(model16, newdata = validationData, type = "response")
summary(validationData$predictions)
...
ggplot() +
  ...
  theme_minimal() +
  labs(
    title = "Predicted occupancy map (2012/13)"
  ) +
...
seasonalSiteCov <- read_csv("processedCovariates/seasonalSiteCov2014_15.csv") %>%
  rename(Pentad = pentad)%>%
  select(-c(1))
...
validationData$urban_prop <- validationData$perc_urban/100
validationData[is.na(validationData)] <- 0
validationData$predictions <- predict(model6, newdata = validationData, type = "response")
summary(validationData$predictions)
